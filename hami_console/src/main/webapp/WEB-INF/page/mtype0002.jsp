...
<head>
    ...
    <script>
         ...
         function dotable(result) {
            //$("#mtype-tbody").append("<tr></tr>")
            //1.获取tbody对象，并清空内容
            var tBody= $("#mtype-tbody");
            tBody.empty();
        ...
        function doCreateTds(row){
            //console.log(row);
            var tds="<td>"+row.tid+"</td>"+
                "<td>"+row.tname+"</td>"+
                "<td>"+row.tdesc+"</td>"+
                "<td><button class='btn btn-sm btn-primary' tyrpe='button' modify  tid="+row.tid+">修改</button>" +
                "<button class='btn btn-sm btn-primary' type='button' modify  tid="+row.tid+">删除</button></td>";
            return tds;
        }

         function doGetObjects(){
             //1.params
             let pageNo=$("#pageId").data("pageNo");
             if(!pageNo)pageNo=1;
             let params={pageNo:pageNo};//暂时默认指定页码值为1
             let tname=$("#tname").html();//获取输入的用户名
             if(tname)params.tname=tname;//tname不能随意写
             //2.url
             let url='/mtype/dofindAll';
             //3.ajax request
             $.ajax({
                 url:url,
                 data:params,
         ...
         function doHandleQueryResponseResult(result){
             if(result.state==1){//1表示ok
                 dotable(result.data.list);//呈现日志记录
                 doSetPagination(result.data);//初始化分页信息
             }
         }
        $(function () {
                // $.ajax({
                //     url:"/mtype/dofindAll",
                //     type:"post",
                //     //data:$("#txForm").serialise(),
                //     success:function(result){
                //         console.log(result.data.list);
                //         dotable(result.data.list);
                //     }
                // });
           $("#toggleSearch").click(function () {
            ...
            $("#pageId").load("/pageination",doGetObjects);
            $(".input-group-btn").on("click","#search",doQueryObjects);
         function doQueryObjects(){
             $("#pageId").data("pageNo",1);
             doGetObjects();
         }
           ...
           $("#addSong").click(function () {
               pop=layer.open({
                   type: 1,
                   area: ['600px', '350px'],
                   content: $('#mtypePop') //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
               });
           })
           ...
           $("[modify]").click(function () {
               ...
               pop1=layer.open({
                   type: 1,
                   area: ['600px', '350px'],
                   content: $('#mtypePop1') //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
               });
...
<div class="wrapper">
    ...
    <div class="page-content">
        <div class="content container">
            ...
            <form id="txForm" action="/mtype/dofindAll" method="post"  class="form-horizontal" >
            <div class="row">
                <div class="col-lg-12">
                    <div class="widget">
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls form-group">
                                        <div class="input-group"> <span class="input-group-addon"><i class="icon-music"></i></span>
                                            <input type="text" placeholder="如：摇滚" name="tname" value="${tname}" id="tname" class="form-control" />
                                    ...
                                    <tbody id="mtype-tbody">
                                      <%--<c:forEach items="${page.list}" var="mtype" varStatus="status" >--%>
                                          <%--<tr>--%>
                                              ...
                                              <%--<td><button class="btn btn-sm btn-primary" type="button" modify  tid="${mtype.tid}"> 修改 </button>--%>
                                                  <%--<button data-toggle="button" class="btn btn-sm btn-warning"  tid="${mtype.tid}"> 删除 </button></td>--%>
                                          <%--</tr>--%>
                                      <%--</c:forEach>--%>
                                    </tbody>
                                </table>
                                <div id="pageId" class="box-footer clearfix" dm="100">
                                    <!--th:include="/common/page.html::page   通过包含碎片来展现-->
                                    <!--th:include="/common/page.html 模板引擎方式-->
                                    <%--<jsp:include page="pageination.jsp"/>--%>
                                </div>
                            </div>
                        </div>
                    </div>
